...
>[!CAUTION]
>Javaのモジュールはセキュリティと保守性を高める(アクセス管理）が、実行時エラーへの耐性（堅牢性）を高める仕組みではない。
...
- カスタムランタイムを作成する **jlink** コマンドを提供している。
      jlink とは、Javaモジュールを組み合わせてアプリ専用の最小実行環境（カスタムランタイム）を作るコマンド
      >背景）  
      >- JAVA8までは、巨大なJREでアプリを動かしていた。
      >- JAVA9以降、JDKはモジュール単位に分割され、アプリを動かすとき、必要なモジュールだけを選べるようになった。
#### 無名モジュール
- JAVA8以前のコードにはモジュール機能がないため、JAVA9以降のコードとの互換性のために、すべてオープンアクセス(requires)となる
- クラスパス上に存在するファイルである
- module-info.javaを持たない
- moduleのセキュリティなどの機能を付与できない
#### モジュール宣言
